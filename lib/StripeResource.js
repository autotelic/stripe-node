...
StripeResource.prototype = {
  ...
  _request(method, host, path, data, auth, options = {}, callback) {
    ...
    const makeRequest = async (apiVersion, headers, numRetries) => {
      ...
      if (this._stripe.getApiField('fetchClient')) {
        try {
          const requestStartTime = Date.now();

          const requestURL = `https://${host ||
            this._stripe.getApiField('host')}${path}`;

          const requestOptions = utils.removeNullish({
            port: this._stripe.getApiField('port'),
            method,
            headers,
            ciphers: 'DEFAULT:!aNULL:!eNULL:!LOW:!EXPORT:!SSLv2:!MD5',
            keepAlive: true,
            body: !utils.isEmptyObject(data)
          ...
          await fetch(requestURL, requestOptions)
            ...
            .then(function(response) {
              ...
              // For convenience, make some headers easily accessible on
              // lastResponse.
              response.requestId = headers['request-id'];
              ...
              // this._recordRequestMetrics(response.requestId, requestDurationMs);
